...
export function downloadReportPdf(
  ...
  const hasBc = (t?: Record<number, number>) =>
    !!t && BC_FREQUENCIES.some((f) => t[f] !== undefined);

  const bcThresholdRow = (
    label: string,
    ac: Record<number, number>,
    bc: Record<number, number>,
    color: string,
    fill: string,
  ) => `
    <tr>
      ...
      ${BC_FREQUENCIES.map((f) => {
        const masked = isMaskingRequired(ac[f], bc[f]);
        return `<td style="background:${fill}">${bc[f]}${masked ? ' <span style="color:#b45309;font-size:9px;font-weight:700;">(mask)</span>' : ''}</td>`;
      }).join('')}
    </tr>`;
...
<div class="section">
  ...
  ${
    hasBc(audiogram.boneRight) || hasBc(audiogram.boneLeft)
      ? `
  <table style="margin-top:12px">
    <thead><tr><th style="text-align:left">Ear (Bone Conduction)</th>${BC_FREQUENCIES.map((f) => `<th>${f} Hz</th>`).join('')}</tr></thead>
    <tbody>
      ${bcThresholdRow('Right', audiogram.right, audiogram.boneRight, '#1d72f1', '#eef7ff')}
      ${bcThresholdRow('Left', audiogram.left, audiogram.boneLeft, '#dc2626', '#fef2f2')}
    </tbody>
  </table>
  <p style="font-size:9px;color:#8d9fb8;margin-top:6px;">"(mask)" indicates an air-bone gap ≥10 dB at that frequency — the unmasked bone reading may not reflect the tested ear.</p>`
      : ''
  }
  <div class="ears">
