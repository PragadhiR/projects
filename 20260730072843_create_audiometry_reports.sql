        ...
        {(hasBc(audiogram.boneRight) || hasBc(audiogram.boneLeft)) && (
          <div className="mt-5 overflow-x-auto">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {([
                  { side: 'Right', ac: audiogram.right, bc: audiogram.boneRight, color: 'text-brand-600' },
                  { side: 'Left', ac: audiogram.left, bc: audiogram.boneLeft, color: 'text-red-600' },
                ] as const).map((row) => (
                  <tr key={row.side} className="border-b border-ink-100 last:border-0">
                    <td className={`py-2.5 text-left text-sm font-bold ${row.color}`}>{row.side}</td>
                    {BC_FREQUENCIES.map((f) => {
                      const masked = isMaskingRequired(row.ac[f], row.bc[f]);
                      return (
                        <td key={f} className="py-2.5 text-center">
                          <span className="font-semibold text-ink-800">{row.bc[f]}</span>
                          {masked && (
                            <span className="mt-0.5 flex items-center justify-center gap-0.5 text-[9px] font-bold text-amber-600">
                              <AlertTriangle className="h-2.5 w-2.5" /> Mask
                            </span>
                          )}
                        </td>
                      );
            ...
            <p className="mt-2 text-[11px] leading-relaxed text-ink-400">
              "Mask" indicates an air-bone gap ≥10 dB at that frequency, where the unmasked bone
              reading may not reflect the tested ear.
            </p>
          </div>
        )}
      </section>
